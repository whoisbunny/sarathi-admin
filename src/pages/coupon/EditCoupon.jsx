...
import { toggleEditModal, editCoupon } from "@/store/coupon/coupon.reducer";
...
const EditCoupon = () => {
  ...
  if (editItem?.endDate) {
    setValue("endDate", dayjs(editItem?.endDate).format("YYYY-MM-DD"))
  }
  if (editItem?.startDate) {
    setValue("startDate", dayjs(editItem?.startDate).format("YYYY-MM-DD"));
  }    setDiscountType(
      DiscountTypeOptions?.find(
        (option) => option.value === editItem.discountType
      )
    );
    setRideTypes(
      editItem.applicableRideTypes?.map((type) =>
        RideTypeOptions.find((option) => option.value === type)
      )
    );
  }, [editItem, reset]);
  ...
  const onSubmit = (data) => {
    ...
    dispatch(editCoupon({ id: editItem._id, data }));
    reset();
