...
const ChatPage = () => {
  ...
  const { activechat, openinfo, mobileChatSidebar, contacts } =
    useSelector((state) => state.chat);



    const { activeUsers, searchContact, socketId } = useSelector(
      (state) => state.socket
    );
    const remainingUsers = activeUsers?.filter((item) => item.id !== socketId);

  // const searchContacts = contacts?.filter((item) =>
  //   item.fullName.toLowerCase().includes(searchContact.toLowerCase())
  // );
  const searchContacts = remainingUsers?.filter((item) =>
    item.username?.toLowerCase().includes(searchContact.toLowerCase())
  );
